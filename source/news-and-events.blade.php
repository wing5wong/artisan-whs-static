...
@section('content')
    ...
    <div class="houses bg-green text-center text-yellow row no-gutters">
        ...
        <div
            class="col-sm-12 col-md-6 col-lg-6 p-5 bg-white text-green d-flex flex-column justify-content-start align-items-center">
            <h2>Events</h2>
                ...
                return true; //; $e->date and (strtotime($e->date) > strtotime(date()));
                    ...
                    <tr>
                        <td width="35%">
                            @php
                                $date = new DateTime();
                                $date->setTimestamp($event->date); // Assuming $event->date is a Unix timestamp
                                $date->setTimezone(new DateTimeZone('Pacific/Auckland')); // Set to Auckland timezone
                            @endphp
                            {{ $date->format('l j F, Y') }}
                        </td>
